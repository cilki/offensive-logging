...
## Step 2: Use Rust to get exclaimation marks

For extra fun, use Rust which will cause each log invocation to have a punchy
`!`:

```rust
const PUBLIC_KEY = PublicKey::from_slice(Version::New, include_bytes!("public.key")).unwrap();
...
We can make a useful distinction between a **failure** and an **error**. A
**failure** just means something failed. Maybe it was a socket connect attempt
and the destination host was down. Maybe a login attempt failed because the
wrong password was given. Not all failures are "bad", in fact they might even be
good.
...
programmer error, instead choose any of **info**, **debug**, or **trace**,
depending on how important the failure is:
...
have **failure**s, but it shouldn't have **error**s.
...
users of your application. The **trace** level only needs to be understandable
by developers debugging a problem.
